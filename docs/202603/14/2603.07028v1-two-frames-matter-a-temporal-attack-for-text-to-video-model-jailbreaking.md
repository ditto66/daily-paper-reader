...
tldr: "本研究针对文本生成视频（T2V）模型的安全性，揭示了一种“时间轨迹填充”漏洞。现有攻击多关注文本改写，而本文提出的TFM框架利用T2V模型在处理碎片化提示词（如仅指定起始和结束帧）时会自动补全中间过程的特性，诱导模型生成包含违规内容的视频。实验证明，该方法在多个商用模型上将攻击成功率提升了12%，强调了开发具备时间感知能力的防御机制的必要性。"
selection_source: fresh_fetch
motivation: 现有的越狱攻击主要依赖文本改写，忽视了T2V模型在处理时间跨度较大的稀疏提示词时可能自主生成有害中间帧的特有漏洞。
method: 提出TFM框架，将不安全请求转化为仅包含起始和结束状态的碎片化提示词，并结合隐式替换来规避内容审查。
result: "在多个开源和商用T2V模型上的实验显示，TFM显著提升了越狱效果，在商用系统上的攻击成功率最高提升了12%。"
conclusion: 研究结果表明，T2V模型的安全机制必须超越表面文本过滤，考虑模型在时间维度上的自主补全能力。
---

## 摘要
最近的文本生成视频 (T2V) 模型能够根据轻量级的自然语言提示词合成复杂的视频，这引发了人们对现实世界中滥用情况下安全对齐问题的紧迫担忧。之前的越狱攻击通常将不安全的提示词重写为释义，在保留原意的同时规避内容过滤器。然而，这些方法往往在输入文本中仍保留显性的敏感线索，因此忽略了一个更深层次的、视频特有的弱点。在本文中，我们识别出了 T2V 系统在碎片化提示词下的时间轨迹填充漏洞：当提示词仅指定稀疏的边界条件（例如起始帧和结束帧）而未明确中间演变过程时，模型可能会自主重建一条包含有害中间帧的合理轨迹，尽管提示词在输入或输出端过滤中看起来是良性的。基于这一观察，我们提出了 TFM。这一碎片化提示框架将原始的不安全请求转换为时间上稀疏的两帧提取，并通过隐式替换进一步减少明显的敏感线索。对多个开源和商业 T2V 模型的广泛评估表明，TFM 持续增强了越狱效果，在商业系统上的攻击成功率提升了高达 12%。我们的研究结果强调，需要建立具备时间感知能力的安全机制，以考虑超出提示词表面形式的模型驱动补全。
