...
tldr: 随着大语言模型的发展，智能体驱动的机器学习流水线搜索成为新趋势，但现有的 Python 生态系统因其顺序执行和解释性限制，难以应对海量流水线的执行需求。本文提出 Stratum 系统基础设施，通过解耦执行与规划、批量编译优化执行图以及引入 Rust 高性能运行时，无缝集成现有 Python 库并显著提升执行效率。Stratum 解决了智能体工作负载与传统 ML 系统之间的不匹配，为大规模自动化机器学习提供了高效支撑。
selection_source: fresh_fetch
motivation: 现有的 Python ML 生态系统主要面向人类交互式工作流，无法高效处理 LLM 智能体在自动化流水线搜索中产生的大规模、高并发执行任务。
method: Stratum 采用解耦执行与规划的架构，将批量流水线编译为优化的执行图，并利用 Rust 编写的高性能运行时在异构后端上高效执行。
result: 初步实验结果显示，Stratum 在处理大规模智能体驱动的机器学习工作负载时，相比现有方案可实现高达 16.6 倍的性能提升。
conclusion: Stratum 成功填补了高性能 ML 系统与 Python 生态系统之间的鸿沟，为未来大规模智能体中心化的机器学习任务提供了必要的基础设施。
---
